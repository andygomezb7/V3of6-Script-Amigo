...
\*********************************/
	...
	// REDEFINIR VARIABLES
	$tsPage = 't.comus_ajax/c.'.$files[$action]['p'];
...
\*********************************/
	...
	switch($action){
		...
		case 'comunidades-responder':
			//<--
			if($_POST['resp']){
			$res = $tsCom->LoadCargRespuestas($_POST['resp']);
			}else{
		...
		case 'comunidades-edit_imagen':
			//<--
				$result2 = $tsCom->uploadImagen($_FILES['imagen'], $_GET['comid'], 2);
				            $uplwad->image_scale = true;
            $uplwad->image_size['w'] = 640;
            $uplwad->image_size['h'] = 480;
            //
            $uplwad->file_url = $_POST['url'];
				$$result = $uplwad->newUpload(1);
