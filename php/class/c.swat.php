...
class tsSwat{
    ...
    */
    function setDenuncia($obj_id, $type = 'posts'){
        ...
        $extras = $w->setSecure($_POST['extras']);
        $date = time();
        // QUE?
        switch($type){
            case 'posts':
            // ¿ES MI POST O ESTÁ EN STICKY?
          $query = mysql_query('SELECT * FROM `noticia` WHERE `id` = \''.(int)$obj_id.'\' LIMIT 1') or die(mysql_error());
          $my_post = mysql_fetch_assoc($query);
            
			if(empty($my_post['id'])) return '0: No puedes denunciar una nota que no existe.';
            if($my_post['id_usuario'] == $_SESSION['uid']) return '0: No puedes denunciar tus propias notas.s';			
			if($my_post['sticky'] == '1') return '0: No puedes denunciar posts en sticky.';
            if($u['rango'] == 1) return '0: No puedes denunciar siendo moderador, pero puedes atender las denuncias de los usuarios.';
            // YA HA REPORTADO?
            $query = mysql_query('SELECT `did` FROM `w_denuncias` WHERE `obj_id` = \''.(int)$obj_id.'\' AND `d_user` = '.$_SESSION['uid'].' AND `d_type` = \'1\'');
            ...
            if(mysql_query('INSERT INTO `w_denuncias` (`obj_id`, `d_user`, `d_razon`, `d_extra`, `d_type`, `d_date`) VALUES (\''.(int)$obj_id.'\', \''.$_SESSION['uid'].'\', \''.$razon.'\', \''.$extras.'\', \'1\', \''.$date.'\')')){
          ...
          $my_photo = mysql_fetch_assoc($query);
            ...
            if($my_photo['f_user'] == $_SESSION['uid']) return '0: No puedes denunciar tus propias fotos.';			
			if($my_photo['f_status'] == '1') return '0: No puedes denunciar fotos ocultas.';
            // YA HA REPORTADO?
            $query = mysql_query('SELECT `did` FROM `w_denuncias` WHERE `obj_id` = \''.(int)$obj_id.'\' AND `d_user` = '.$_SESSION['uid'].' AND `d_type` = \'4\'');
            $denuncio = mysql_num_rows($query);
            ...
            // INSERTAR NUEVA DENUNCIA
            if(mysql_query('INSERT INTO `w_denuncias` (`obj_id`, `d_user`, `d_razon`, `d_extra`, `d_type`, `d_date`) VALUES (\''.(int)$obj_id.'\', \''.$_SESSION['uid'].'\', \''.$razon.'\', \''.$extras.'\', \'4\', \''.$date.'\')')){
				return '1: La denuncia fue enviada.';
            } else return '0: Error int&eacute;ntalo m&aacute;s tarde.';
            ...
            case 'mensaje':
                // YA HA REPORTADO?
                $query = mysql_query('SELECT `did` FROM `w_denuncias` WHERE `obj_id` = \''.(int)$obj_id.'\' AND d_user = \''.$_SESSION['uid'].'\' AND `d_type` = \'2\'');
                $denuncio = mysql_num_rows($query);
                ...
                //
                if($where['mp_to'] == $_SESSION['uid']) $del_table = 'mp_del_to';
                elseif($where['mp_from'] == $_SESSION['uid']) $del_table = 'mp_del_from';
                // INSERTAR NUEVA DENUNCIA
				if(mysql_query('INSERT INTO w_denuncias (obj_id, d_user, d_razon, d_extra, d_type, d_date) VALUES (\''.(int)$obj_id.'\', \''.$_SESSION['uid'].'\', \'0\', \'\', \'2\', \''.$date.'\') ')){
                   // BORRAMOS
			...
			case 'comunidad':
				...
				if($my_comu['c_autor'] == $_SESSION['uid']) return '0: No puedes denunciar tus propias comunidades.';			
				if($my_comu['c_estado'] == '1') return '0: No puedes denunciar comunidades ocultas.';
				// YA HA REPORTADO?
				$query = mysql_query('SELECT `did` FROM `w_denuncias` WHERE `obj_id` = \''.(int)$obj_id.'\' AND `d_user` = '.$_SESSION['uid'].' AND `d_type` = \'5\'');
				$denuncio = mysql_num_rows($query);
				
				if(!empty($denuncio)) return '0: Ya hab&iacute;as denunciado esta comunidad.';
				// INSERTAR NUEVA DENUNCIA
				if(mysql_query('INSERT INTO `w_denuncias` (`obj_id`, `d_user`, `d_razon`, `d_extra`, `d_type`, `d_date`) VALUES (\''.(int)$obj_id.'\', \''.$_SESSION['uid'].'\', \''.$razon.'\', \''.$extras.'\', \'5\', \''.$date.'\')')){
			...
			case 'tema':
				...
				if($my_tema['t_autor'] == $_SESSION['uid']) return '0: No puedes denunciar tus propios temas.';			
				if($my_tema['t_estado'] == '1') return '0: No puedes denunciar temas ocultos.';
				// YA HA REPORTADO?
				$query = mysql_query('SELECT `did` FROM `w_denuncias` WHERE `obj_id` = \''.(int)$obj_id.'\' AND `d_user` = '.$_SESSION['uid'].' AND `d_type` = \'6\'');
				$denuncio = mysql_num_rows($query);			
				...
				// INSERTAR NUEVA DENUNCIA
				if(mysql_query('INSERT INTO `w_denuncias` (`obj_id`, `d_user`, `d_razon`, `d_extra`, `d_type`, `d_date`) VALUES (\''.(int)$obj_id.'\', \''.$_SESSION['uid'].'\', \''.$razon.'\', \''.$extras.'\', \'6\', \''.$date.'\')')){
				return '1: La denuncia fue enviada.';
				} else return '0: Error, int&eacute;ntalo m&aacute;s tarde.';
			break;
            // USUARIOS
            case 'usuario':
                // YA HA REPORTADO?
				$query = mysql_query('SELECT did FROM w_denuncias WHERE obj_id = \''.(int)$obj_id.'\' AND d_user = \''.$_SESSION['uid'].'\' AND d_type = \'3\'');
                $denuncio = mysql_num_rows($query);
                
                if(!empty($denuncio)) return '0: Ya hab&iacute;as denunciado a este usario.';
                $username = $wuser->loadUserN($obj_id);
                if(empty($username)) return '0: Opps... Este usuario no existe.';
                // LO REPORTAMOS...
				if(mysql_query('INSERT INTO w_denuncias (obj_id, d_user, d_razon, d_extra, d_type, d_date) VALUES (\''.(int)$obj_id.'\', \''.$_SESSION['uid'].'\', \''.$w->setSecure($razon).'\', \''.$w->setSecure($extras).'\', \'3\', \''.$date.'\')')){
				 // SUMAMOS
                   // mysql_query('UPDATE `usuarios` SET `user_bad_hits` = user_bad_hits + 1 WHERE `usuario_id` = \''.(int)$obj_id.'\'');
                    return '1: Este usuario ha sido denunciado.';
                } else return '0: Error! Int&eacute;ntalo m&aacute;s tarde.';
            break;
            case 'bwort':
                            // YA HA REPORTADO?
                $query = mysql_query('SELECT did FROM w_denuncias WHERE obj_id = \''.(int)$obj_id.'\' AND d_user = \''.$_SESSION['uid'].'\' AND d_type = \'7\'');
                $denuncio = mysql_num_rows($query);
                
                if(!empty($denuncio)) return '0: Ya hab&iacute;as denunciado este Bwort.';
                $username = $wuser->loadUserN($obj_id);
                if(empty($username)) return '0: Opps... Este usuario no existe.';
                // LO REPORTAMOS...
                if(mysql_query('INSERT INTO w_denuncias (obj_id, d_user, d_razon, d_extra, d_type, d_date) VALUES (\''.(int)$obj_id.'\', \''.$_SESSION['uid'].'\', \''.$w->setSecure($razon).'\', \''.$w->setSecure($extras).'\', \'7\', \''.$date.'\')')){
                 // SUMAMOS
                   // mysql_query('UPDATE `usuarios` SET `user_bad_hits` = user_bad_hits + 1 WHERE `usuario_id` = \''.(int)$obj_id.'\'');
                    return '1: Este Bwort ha sido denunciado.';
                } else return '0: Error! Int&eacute;ntalo m&aacute;s tarde.';
            break;
        }
    }
