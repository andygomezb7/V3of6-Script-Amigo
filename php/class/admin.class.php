...
$data = mysql_fetch_assoc($query);
	...
	**/
	...
	'rgbwrts' => $w->setSecure($_POST['wr']),
	);

	if($edu['rgbwrts'] && $edu['rgbwrts'] > 0){
	$wuser->darPuntos($wuser->uid, $edu['rgbwrts'], 4);
	}else{}
    ...
    `worts` = \"".$wotsInrK."\" + \"".(int)$edu['rgbwrts']."\",
...
//=> MEDALLAS
	...
	/****************** ADMINISTRACIÓN DE BUGS, DENUNCIAS, OPCIONES DE PERFILES ******************/
      
    function getModActivida(){
    	global $tsActividad;
    	return $tsActividad->getModActividad();
    }

    function verificar($userid, $type){
    	global $wuser, $tsActividad;
    	$isverifi = mysql_fetch_assoc(mysql_query("SELECT * FROM usuarios WHERE usuario_id='".$userid."'"));
    	if($type == 1){ $vari = 'Verificado'; }else{ $vari = 'DesVerificado'; }
    	if($isverifi['verifi'] == $type){
    		$data = '0: Este usuario ya esta '.$vari.' ::'.$type;
    	}else{
    	if(mysql_query("UPDATE usuarios SET verifi='".$type."' WHERE usuario_id='".$userid."'")){
    		if($type == 1){ $var = 75; }else{ $var = 77; }
    		$tsActividad->setModActividad($var, $userid);
    	$data = '1: Verificada';
    	}else{ $data = '0: Ocurrio un error, intenta nuevamente.'; }
        }
    	return $data;
    }

    function bannear($userid, $type){
    	global $wuser, $tsActividad;
    	   $isbaned = mysql_fetch_assoc(mysql_query("SELECT * FROM usuarios WHERE usuario_id='".$userid."'"));
    	   if($type == 1){ $vari = 'Banneado'; }else{ $vari = 'DesBanneado'; }
    	if($isbaned['banned'] == $type){
    		$data = '0: Este usuario ya esta '.$vari.' ::'.$type;
    	}else{
    	if(mysql_query("UPDATE usuarios SET banned='".$type."' WHERE usuario_id='".$userid."'")){
    		if($type == 1){ $var = 76; }else{ $var = 78; }
    		$tsActividad->setModActividad($var, $userid);
    	$data = '1: Baneado';
    	}else{ $data = '0: Ocurrio un error, intenta nuevamente.'; }
        }
	...
	function getDenuncias(){
        ...
        $e = 0; foreach($tsDenuncias[$tipoes] as $rey){
        if($row['d_razon'] == $e){ $data[$e]['razon'] = $rey; }else{ $data[$e]['razon'] = 'Indefinida'; }
        $e++;
         }
        $data[$i]['user'] = mysql_fetch_assoc(mysql_query("SELECT * FROM usuarios WHERE usuario_id='".$row['d_user']."'"));
        $i++;
        }
...
/*----------------------------------------- 
ADMINISTRAR ADS PUBLICIDAD
-----------------------------------------*/

function loadAdsW($type, $obj){
switch ($type) {
case 1:
$query = mysql_query("SELECT ads.*, u.* FROM u_ads AS ads LEFT JOIN usuarios AS u ON ads.ua_user = u.usuario_id WHERE ads.au_code='".$obj."'");
$data = result_array($query);
break;
}
return $data;
...
/*----------------------------------------- 
END; ADMINISTRAR ADS PUBLICIDAD
-----------------------------------------*/
...
function cssTot($type = 1, $name){
if($type == 1){ // LOAD TOT ARCHIVES
$arr_ext = array('css');
$mydir = opendir(TS_ROOT.'/css/'.$f);
while($file = readdir($mydir)){
$ext = substr($file, -3);
if(in_array($ext, $arr_ext)){
if(!empty($size)){
