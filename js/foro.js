...
var com = {
	...
	subcat: function() {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url +'/ajax/comunidades-subcategorias.php',
			data: 'cid='+cid,
			success: function(h){
	...
	crear_tema: function() {
		...
		if(com.required()) {
            ...
            mydialog.center();
			$("#KSJAkalaskdnAS").html('gola esta subiendose jeje');
			$("#loading").show();
            // PREVIEW
			$.ajax({
				type: 'post',
				url: global_data.url + '/ajax/comunidades-preview_tema.php',
				data: 'titulo=' + encodeURIComponent($('input[name=titulo]').val()) + '&cuerpo=' + encodeURIComponent($('#wysibb').val()),
				success: function(h) {
					mydialog.body(h);
					$("#KSJAkalaskdnAS").html(h);
	...
	borrar:function(comid, gew){
		if(!gew){
			...
			$.ajax({
				type: 'post',
				url: global_data.url + '/ajax/comunidades-borrar_com.php',
				data: 'comid=' + comid + '&susp=1',
				success: function(h) {
					mydialog.alert((h.charAt(1) == '0' ? 'Opps!' : 'Hecho'), h.substring(3), true);
				}
			});
			$('#loading').fadeOut(350);
	...
	ir_a_categoria: function(cat) {
		...
		if(cat==-1) document.location.href= global_data.url + '/foro/';
		else document.location.href= global_data.url + '/foro/home/' + cat + '/';
	},
	...
	add_respuesta: function() {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-responder.php',
	...
	unirme: function() {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-unirme.php',
	...
	abandonar: function(gew) {
		...
		$('#loading').fadeIn(250);
			$.ajax({
				type: 'POST',
				url: global_data.url + '/ajax/comunidades-abandonar.php',
				data: cgget('comid', true),
				success: function(h){
					if(h.charAt(1) == 1) {
						$('.action_comunidad').toggle();
	...
	seguir_com: function() {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-seguir_com.php',
			data: cgget('comid', true),
			success: function(h){
				$('#loading').fadeOut(250);
				if(h.charAt(1) == '1') {
	...
	seguir_tema: function() {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-seguir_tema.php',
	...
	subir_imagen: function() {
		...
		$('#loading').fadeIn(250);
				...
				$.ajaxFileUpload({
					url: global_data.url + '/ajax/comunidades-edit_imagen.php?' + cgget('comid', true),
	...
	subir_fondo: function() {
		...
		if ($('#cef_input').val()) {
			if(isImageFile($('#cef_input').val())){
				...
				$.ajaxFileUpload({
					url: global_data.url + '/ajax/comunidades-edit_fondo.php?' + cgget('comid', true),
	...
	votar_tema: function(voto) {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-votar_tema.php',
	...
	votar_respuesta: function(rid, voto) {
		$('#loading').fadeIn(250);
		voto = (voto == 'pos') ? 'pos' : 'neg';
		var cmbird = (voto == 'pos') ? 'like' : 'dislike';
		var comm = $('#total_votos_' + rid);
		var total_votos = parseInt(comm.text());
        total_votos = (isNaN(total_votos)) ? 0 : total_votos;
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-votar_respuesta.php',
			...
			success: function(h){
				...
				$('#coment_id_'+rid+' .ccr_options .i_'+cmbird).removeClass('icon_'+cmbird);
				$('#coment_id_'+rid+' .ccr_options .i_'+cmbird).addClass('icon_'+cmbird+'_eject');
			}
		});
		$('#loading').fadeOut(250);
	},
	add_favorito: function() {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-add_favorito.php',
			data: cgget('temaid'),
			success: function(h){
				if(h.charAt(1) == 1) {
					$('.add_favorito').toggle();
	...
	reco_tema: function (gew) {
		if(!gew) {
			...
			$.ajax({
				type: 'POST',
				url: global_data.url + '/ajax/comunidades-reco_tema.php',
				data: cgget('temaid'),
				success: function(h){
					mydialog.procesando_fin();
					if(h.charAt(0) == 1) {
	...
	del_tema: function(gew) {
		if(!gew) {
			...
			$.ajax({
				type: 'POST',
				url: global_data.url + '/ajax/comunidades-del_tema.php',
				data: cgget('temaid', true),
				success: function(h){
					mydialog.procesando_fin();
					if(h.charAt(0) == 1) {
	...
	del_mod_tema: function(gew) {
		if(!gew) {
			...
			mydialog.loading('Eliminado tema');
			$.ajax({
				type: 'POST',
				url: global_data.url + '/ajax/comunidades-del_tema.php',
	...
	reactivar_tema: function(gew) {
		if(!gew) {
			...
			$.ajax({
				type: 'POST',
				url: global_data.url + '/ajax/comunidades-reactivar_tema.php',
				data:  cgget('temaid', true),
				success: function(h){
					mydialog.procesando_fin();
					if(h.charAt(0) == 1) {
	...
	del_respuesta: function(rid) {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-del_respuesta.php',
			data: 'rid=' + rid,
			success: function(h){
				$('#coment_id_'+rid).remove();			
				if(h.charAt(0) == 1) {
	...
	pages_temas: function(page) {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-pages_temas.php',
	...
	pages_respuestas: function(page) {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-pages_respuestas.php',
	...
	member_admin_submit: function() {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-member_admin_submit.php',
	...
	load_members: function(type) {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-load_members.php',
	...
	admin_rehabilitar: function(userid, gew) {
		if(!gew) {
			...
			$.ajax({
				type: 'POST',
				url: global_data.url + '/ajax/comunidades-member_reactivar.php',
	...
	save_borrador: function() {
		if(com.required()) {
			...
			$.ajax({
				type: 'POST',
				url: global_data.url + '/ajax/comunidades-save_borrador.php',
	...
	del_favorito: function(favid) {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-del_favorito.php',
			data: 'favid=' + favid,
			success: function(h){
				if(h.charAt(0) == 1) {
	...
	del_borrador: function(bid) {
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-del_borrador.php',
			data: 'bid=' + bid,
			success: function(h){
				if(h.charAt(0) == 1) {
...
nuevos: function(mostrar_resp, comentarionum, cid){
		...
		$.ajax({
			type: 'POST',
			url: global_data.url + '/ajax/comunidades-responder.php',
			data: 'body=' + encodeURIComponent(text) + '&resp=' + cid + cgget('temaid'),
			success: function(h){
				if(h.charAt(1) == '0'){
					$('.com_bigmsj_red_'+cid).html(h.substring(3)).show('fast');
					$('#btn_newcom').removeAttr('disabled');
				} else {					
					$('#markit_resp').val('');
					$('#add_new_com').html(h).slideDown('slow', function () {
						$('.com_bigmsj_blue').hide('fast');
						$('.com_bigmsj_red_'+cid).hide('fast');
						$(this).attr({'id' : 'old_msj'});
					});
					// SUMAMOS
					var com_total = parseInt($('#cbt_val').text());
